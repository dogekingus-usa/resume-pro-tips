...
check('title', /<title>[^<]*ResumeProTips/i.test(home), (home.match(/<title>([^<]*)<\/title>/) || [])[1]);
...
check('theme-resumeprotips', home.includes('class="theme-resumeprotips"'));
check('canonical', /rel="canonical" href="https:\/\/resumeprotips\.com\/?"/.test(home), (home.match(/rel="canonical" href="([^"]+)"/) || [])[1]);

console.log('=== ARTICLE PAGE ===');
const art = fs.readFileSync(path.join(dist, '10-mistakes-that-make-your-resume-look-dated-2026-edition.html'), 'utf8');
check('article title', /<title>[^<]*Resume Look Dated/i.test(art), (art.match(/<title>([^<]*)<\/title>/) || [])[1]);
...
check('canonical flat', art.includes('rel="canonical" href="https://resumeprotips.com/10-mistakes-that-make-your-resume-look-dated-2026-edition"'));
